...
(
    id                INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    severity_level_id INTEGER REFERENCES config.owner (id),
    event_type_id     INTEGER REFERENCES config.event_type (id),
    case_id           INTEGER REFERENCES data.case (id),
    timestamp         TIMESTAMP,
    title             VARCHAR(255),
    details           VARCHAR(2048)
);
...
CREATE TABLE data.events_services
(
    event_id   INTEGER REFERENCES data.event,
